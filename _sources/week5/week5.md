...
An alternative way to write {eq}`eq49` is using the density form, where the density is defined as $\rho=\frac{1}{\alpha}$ (bigger ensemble spread, less likely to have observe members at a fixed volume). Thus, a more widely used definition of information is $-\mathrm{ln}\rho(x_{i})$.
